...
from models import db, Problem, Case, Submission, Student, User, ProblemTopic, Statistic
...
class EvaluatorAttemptSubmission(Resource):
    ...
    def post(self):
        ...
        stat = Statistic.query.filter(Statistic.problem_id == problem_id).one()
        stat.total_submissions = stat.total_submissions + 1.0
        db.session.commit()
...
class EvaluatorProblemSubmissionResult(Resource):
    ...
    def post(self):
        ...
        stat = Statistic.query.filter(Statistic.problem_id == problem.id).one()
        if update_data['grade'] == 100:
            stat.total_accepted_submissions = stat.total_accepted_submissions + 1.0
        db.session.commit()
        #############
