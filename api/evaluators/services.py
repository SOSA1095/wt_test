...
# base_dir = "/Users/danielgonzalez/Documents/Work/wt_247/Evaluator/"
base_dir = "/var/www/wt_247/Evaluator/"
# base_dir = "/Users/jorgevazquez/Documents/wt_247/Evaluator/"
...
def evaluate(request):
    ...
    for test_no in range(total_tests):
        ...
        # 6) Execute
        process = subprocess.Popen(['docker', 'exec', ctr_name, 'python3', 'monitor.py', str(language), str(time_limit), str(memory_limit)],
            stdout=subprocess.PIPE, stderr=subprocess.PIPE)

        try:
            status, stderrout = process.communicate(timeout=5)
        ...
        if (request_type == "submission"):
            if (resultStatus == "successful run"):
        ...
        # Return output as is
            ...
            if (resultStatus == "successful run"):
                results.append({ "status" : resultStatus, "output" : output })
            else:
                results.append({ "status" : resultStatus })
    ...
    return_obj["time"] = status["time"]

    if (request_type == "submission"):
        return_obj["status"] = SubmissionState.evaluated.value
        return_obj["submission_id"] = submission_id
        return_obj["update"] = requests.post("http://localhost:5000/api/evaluator/problem_submission_result", json=return_obj)
